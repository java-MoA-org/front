...
export const getUserPageRequest = async (nickname: string, accessToken: string) => {
...
export const passwordVerifyRequest = async (accessToken: string, body: PasswordVerifyRequestDto) => {
  const responseBody = await axios
...
export const patchPasswordUserPageRequest = async (accessToken: string, body: PatchPasswordUserPageRequestDto) => {
  const responseBody = await axios
...
export const patchUserInfoRequest = async (accessToken: string, requestBody: PatchUserInfoRequestDto) => {
  const responseBody = await axios
...
export const patchTransactionStatusRequest = async (
  tradeSequence: number | string,
  updateStatus: string,
  accessToken: string
) => {
  const responseBody = await axios
    .patch(PATCH_USED_TRADE_TRANSACTION_URL(tradeSequence, updateStatus), null, bearerAuthorization(accessToken))
    .then(responseSuccessHandler)
    .catch(responseErrorHandler);
  return responseBody;
};
